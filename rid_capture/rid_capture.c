...
int main(int argc, char* argv[])
{
  ...
  if (pcap_lookupnet(wifi_name,&network,&netmask,errbuf) < 0) {
    ...
    setup_ms = ((double)setup_done.tv_sec * 1e3 + (double)setup_done.tv_nsec * 1e-6) - ((double)start.tv_sec * 1e3 + (double)start.tv_nsec * 1e-6);
    ...
    while (!end_program) {
        ...
        loop_us = ((double)loop_entry.tv_sec * 1e6 + (double)loop_entry.tv_nsec * 1e-3) - ((double)last_loop.tv_sec * 1e6 + (double)last_loop.tv_nsec * 1e-3);
...
void packet_handler(u_char* args, const struct pcap_pkthdr* header, const u_char* packet)
{
    ...
    if (payload[0] == 0x80) { // beacon
        ...
        while (offset < length) {
            ...
            } else if ((typ == 0xdd) && (val[0] == oui_alliance[0]) && // WiFi Alliance
    ...
    write_json("\" }\n");
        ...
        if (memcmp(nan_cluster, &payload[16], 6) == 0) { // NAN
            ...
            if ((length > 44) && (payload[offset] == 0x04) && (payload[offset + 1] == 0x09) && (payload[offset + 2] == oui_alliance[0]) && (payload[offset + 3] == oui_alliance[1]) && (payload[offset + 4] == oui_alliance[2]) && (memcmp(nan_service, &payload[offset + 9], 6) == 0)) {
...
void print_help()
{
    ...
    int i;
    static const char *help_text[] = { "Options",
        "",
#if BLUEZ_SNIFFER
        "-b <bluez device>",
#endif
