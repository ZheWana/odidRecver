 ...
 *
 * Developed on -
 * 1) an NC10 running Debian using its native WiFi hardware and a ZyXEL AC1200
 * (RTL2812AU), 2) a Raspberry Pi running Raspbian using a ZyXEL AC1200.
 *
 * Don't compile with -std=c99, libpcap doesn't like it.
 *
 * Pi
 * RTL8812AU driver 88XXAU, trying to set monitor mode via libpcap causes an
 * error.
 *
 * Keep an eye on the main loop time. It may require tuning if you are using
 * the display or exporting.
 *
 *
 * To Do
 *
...
static const char default_key[] = "0123456789abcdef", default_iv[] = "nopqrs",
                  ...
                  debug_filename[] = "debug.txt", device_pi[] = "wlan0",
                  device_i686[] = "wlp5s0b1",
                  default_log_dir[] = "/tmp/rid_capture",
                  default_www_dir[] = "www", dummy[] = "";
const char pass_s[] = "Pass", fail_s[] = "Fail";
static volatile uint32_t rx_packets = 0, odid_packets = 0;
#ifdef __USING_POSIX_MSGQUEUE
static mqd_t g_msg_queue = (mqd_t)-1; // add global message queue descriptor
static uint16_t channel_freq = 0;
#endif
...
static const char *filter_text
    = "ether broadcast or ether dst 51:6f:9a:01:00:00 ";
#endif
...
main (int argc, char *argv[])
{
  ...
  char *arg, *wifi_name, *nrf_name, *bluez_name, *udp_server, text[128];
...
#ifdef __USING_POSIX_MSGQUEUE
  /* Open existing POSIX message queue for ODID packets (no create) */
  g_msg_queue = mq_open (MSG_QUEUE_NAME, O_WRONLY | O_NONBLOCK);
  if (g_msg_queue == (mqd_t)-1)
    {
      fprintf (stderr, "mq_open(%s) failed: %d (%s)\n", MSG_QUEUE_NAME, errno,
               strerror (errno));
      /* 队列既然应该已存在，如果打开失败通常是严重问题；
         按你的需求选择：可以直接 exit(1) 或仅告警继续运行。*/
      // exit(1);
    }
#endif
  ...
  if (i = pcap_activate (session))
    {
      fprintf (stderr, "\npcap_activate():  %s, %s\n", pcap_geterr (session),
               pcap_strerror (i));
      fputs ("This error may mean that you don\'t have permission to access "
             "your wifi hardware or that it is not capable of being put into "
             "monitor mode.\n",
             stderr);

      if (set_monitor)
        {
          fputs ("Try setting monitor mode using iw and using the -x option "
                 "to this program.\n",
                 stderr);
        }
  ...
  if ((header_type = pcap_datalink (session)) != DLT_IEEE802_11_RADIO)
    {
      fprintf (stderr,
               "pcap_datalink(): Not tested on this header type. ^C now to "
               "abort. : %d\n",
               header_type);
    }
  ...
  setup_ms
      = ((double)setup_done.tv_sec * 1e3 + (double)setup_done.tv_nsec * 1e-6)
  ...
  while (!end_program)
    {
      ...
      loop_us = ((double)loop_entry.tv_sec * 1e6
                 + (double)loop_entry.tv_nsec * 1e-3)
                - ((double)last_loop.tv_sec * 1e6
                   + (double)last_loop.tv_nsec * 1e-3);
      ...
      if (ble_sniffer > -1)
        ...
        // The nRF sniffer can put out a lot of data hence the separate
        // process.

#if NRF_SNIFFER
      if (nrf_child > 0)
        {
          for (nrf_reads = 0, nrf_bytes = 0;
               (nrf_reads < 4) && (!end_program);)
            {

              if ((bytes = read (nrf_pipe, nrf_buffer, sizeof (nrf_buffer)))
                  > 0)
                {
      ...
      if ((secs - last_debug) > 9)
        {
          ...
          sprintf (text,
                   "{ \"debug\" : \"rx packets %u (%u)\", \"loop_time_us\" : "
                   "%.0f, \"max udp len\" : %d }\n",
...
#ifdef __USING_POSIX_MSGQUEUE
  /* POSIX message queue cleanup: close only, DO NOT unlink (queue created
   * elsewhere) */
  if (g_msg_queue != (mqd_t)-1)
    {
 ...
 * @param args User-provided string (not used)
 * @param header Packet header information
 * @param packet Captured packet data
 *
 * Processes a captured packet, extracting OUI, Beacon, and NAN information
 * and printing it out in JSON format.
 */

void
packet_handler (u_char *args, const struct pcap_pkthdr *header,
                const u_char *packet)
{
...
#ifdef __USING_POSIX_MSGQUEUE
  channel_freq = *(u_int16_t *)&packet[18];
#endif
  ...
  if (payload[0] == 0x80)
      ...
      while (offset < length)
        {
          ...
          if ((typ == 0xdd) && (val[0] == 0xfa) && // ODID
              (val[1] == 0x0b) && (val[2] == 0xbc))
            {

              parse_odid (mac, &payload[offset + 6], length - offset - 6, 0,
                          "pcap beacon", NULL);
            }
          else if ((typ == 0xdd) && (val[0] == oui_alliance[0])
                   && // WiFi Alliance
                   (val[1] == oui_alliance[1]) && (val[2] == oui_alliance[2]))
            {
#if 0
        write_json("{ \"debug\" : \"Beacon with Alliance OUI\" }\n");
          ...
          else if ((typ == 0xdd) && (val[0] == 0x6a) && // French ID
                   (val[1] == 0x5c) && (val[2] == 0x35))
            {
#if ID_FRANCE
    ...
    write_json("\" }\n");
      ...
      if (memcmp (nan_cluster, &payload[16], 6) == 0)
        { // NAN

          offset = 24;

          if ((length > 44) && (payload[offset] == 0x04)
...
getTimeStepMs (void)
{
  ...
  if (clock_gettime (CLOCK_MONOTONIC, &ts) != 0)
    {
      return 0; // 出错时返回 0，按需调整
    }

  // 注意：ts.tv_sec 是 time_t，乘 1000
  // 可能溢出，但一般系统运行时间不会大到超出 int 范围。
...
void
parse_odid (u_char *mac, u_char *payload, int length, int rssi,
            const char *note, const float *volts)
{
  ...
  ODID_MessagePack_encoded *encoded_data
      = (ODID_MessagePack_encoded *)&payload[1];
  ...
  switch (payload[1] & 0xf0)
    ...
    case 0x00:
      decodeBasicIDMessage (&UAS_data.BasicID[0],
                            (ODID_BasicID_encoded *)&payload[1]);
      UAS_data.BasicIDValid[0] = 1;
      break;

    case 0x10:
      decodeLocationMessage (&UAS_data.Location,
                             (ODID_Location_encoded *)&payload[1]);
      UAS_data.LocationValid = 1;
      break;

    case 0x20:
      page = payload[2] & 0x0f;
      decodeAuthMessage (&UAS_data.Auth[page],
                         (ODID_Auth_encoded *)&payload[1]);
      UAS_data.AuthValid[page] = 1;
      break;

    case 0x30:
      decodeSelfIDMessage (&UAS_data.SelfID,
                           (ODID_SelfID_encoded *)&payload[1]);
      UAS_data.SelfIDValid = 1;
      break;

    case 0x40:
      decodeSystemMessage (&UAS_data.System,
                           (ODID_System_encoded *)&payload[1]);
      UAS_data.SystemValid = 1;
      break;

    case 0x50:
      decodeOperatorIDMessage (&UAS_data.OperatorID,
                               (ODID_OperatorID_encoded *)&payload[1]);
  ...
  sprintf (json, "{ \"mac\" : \"%02x:%02x:%02x:%02x:%02x:%02x\"", mac[0],
  ...
  if (UAS_data.OperatorIDValid)
    {
      ...
      sprintf (json, ", \"operator\" : \"%s\"",
               UAS_data.OperatorID.OperatorId);
      write_json (json);

      memcpy (&RID_data[RID_index].odid_data.OperatorID, &UAS_data.OperatorID,
      ...
      if (UAS_data.BasicIDValid[j])
        {
          ...
          memcpy (&RID_data[RID_index].odid_data.BasicID[j],
                  &UAS_data.BasicID[j], sizeof (ODID_BasicID_data));

          switch (UAS_data.BasicID[j].IDType)
            {

            case ODID_IDTYPE_SERIAL_NUMBER:
              memcpy (&RID_data[RID_index].basic_serial, &UAS_data.BasicID[j],
                      sizeof (ODID_BasicID_data));
              sprintf (json, ", \"uav id\" : \"%s\"",
                       UAS_data.BasicID[j].UASID);
              write_json (json);
#if 0
        display_identifier(RID_index + 1,UAS_data.BasicID[j].UASID);
            ...
            case ODID_IDTYPE_CAA_REGISTRATION_ID:
              memcpy (&RID_data[RID_index].basic_caa_reg, &UAS_data.BasicID[j],
                      sizeof (ODID_BasicID_data));
              sprintf (json, ", \"caa registration\" : \"%s\"",
                       UAS_data.BasicID[j].UASID);
              write_json (json);
              display_identifier (RID_index + 1, UAS_data.BasicID[j].UASID);
              break;
  ...
  if (UAS_data.LocationValid)
    {
      ...
      sprintf (json, ", \"uav altitude\" : %d, \"uav heading\" : %d",
               (int)UAS_data.Location.AltitudeGeo,
    ...
    write_json(json);
      ...
      sprintf (json, ", \"uav speed\" : %d, \"seconds\" : %d",
               ...
               (int)UAS_data.Location.TimeStamp);
      write_json (json);

      memcpy (&RID_data[RID_index].odid_data.Location, &UAS_data.Location,
              sizeof (ODID_Location_data));

      display_uav_loc (RID_index + 1, latitude, longitude, (int)altitude,
                       (int)UAS_data.Location.TimeStamp);

      if ((latitude < RID_data[RID_index].min_lat)
          || (RID_data[RID_index].min_lat == 0.0))
        {
          RID_data[RID_index].min_lat = latitude;
        }

      if ((latitude > RID_data[RID_index].max_lat)
          || (RID_data[RID_index].max_lat == 0.0))
        {
          RID_data[RID_index].max_lat = latitude;
        }

      if ((longitude < RID_data[RID_index].min_long)
          || (RID_data[RID_index].min_long == 0.0))
        {
          RID_data[RID_index].min_long = longitude;
        }

      if ((longitude > RID_data[RID_index].max_long)
          || (RID_data[RID_index].max_long == 0.0))
        {
      ...
      if (altitude > INV_ALT)
        {
          ...
          if ((altitude < RID_data[RID_index].min_alt)
              || (RID_data[RID_index].min_alt == 0.0))
            {
              RID_data[RID_index].min_alt = altitude;
            }

          if ((altitude > RID_data[RID_index].max_alt)
              || (RID_data[RID_index].max_alt == 0.0))
            {
  ...
  if (UAS_data.SystemValid)
    {
      ...
      sprintf (
          json, ", \"base latitude\" : %11.6f, \"base longitude\" : %11.6f",
          UAS_data.System.OperatorLatitude, UAS_data.System.OperatorLongitude);
      write_json (json);
      sprintf (json, ", \"unix time\" : %lu",
               ((unsigned long int)UAS_data.System.Timestamp)
                   + ID_OD_AUTH_DATUM);
      write_json (json);

      memcpy (&RID_data[RID_index].odid_data.System, &UAS_data.System,
              sizeof (ODID_System_data));

      display_timestamp (RID_index + 1,
                         (time_t)UAS_data.System.Timestamp + ID_OD_AUTH_DATUM);
    }
  ...
  if (UAS_data.SelfIDValid)
    {
      ...
      memcpy (&RID_data[RID_index].odid_data.SelfID, &UAS_data.SelfID,
      ...
      if (UAS_data.AuthValid[page])
        {
          ...
          if (page == 0)
            {
              ...
              sprintf (json, ", \"unix time (alt)\" : %lu",
                       ((unsigned long int)UAS_data.Auth[page].Timestamp)
                           + ID_OD_AUTH_DATUM);
              write_json (json);

              display_timestamp (RID_index + 1,
                                 (time_t)UAS_data.Auth[page].Timestamp
                                     + ID_OD_AUTH_DATUM);
            }

          sprintf (json, ", \"auth page %d\" : { \"text\" : \"%s\"", page,
                   printable_text (UAS_data.Auth[page].AuthData,
                                   (page) ? ODID_AUTH_PAGE_NONZERO_DATA_SIZE
          ...
          for (i = 0; i < ((page) ? ODID_AUTH_PAGE_NONZERO_DATA_SIZE
                                  : ODID_AUTH_PAGE_ZERO_DATA_SIZE);
               ++i)
            {

              sprintf (json, "%s %d", (i) ? "," : "",
                       UAS_data.Auth[page].AuthData[i]);
              write_json (json);
            }
          ...
          memcpy (&RID_data[RID_index].odid_data.Auth[page],
                  &UAS_data.Auth[page], sizeof (ODID_Auth_data));
        }
    }
...
#ifdef __USING_POSIX_MSGQUEUE
  /* ------------------------------------------------------------------
   * Send parsed ODID data via existing POSIX message queue
   * ID      : MAC address string
   * timestep: system running time in ms (CLOCK_MONOTONIC)
   * ------------------------------------------------------------------ */
  if (g_msg_queue != (mqd_t)-1)
    {
      msgPacket_t pkt;
      ...
      /* 用 MAC 地址作为 UAV 唯一标识 */
      snprintf (pkt.ID, sizeof (pkt.ID), "%02x:%02x:%02x:%02x:%02x:%02x",
                mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
      /* 使用系统运行时间 (ms) */
      pkt.timestep = getTimeStepMs ();
      pkt.channel_freq = channel_freq;
      if (mq_send (g_msg_queue, (const char *)&pkt, sizeof (pkt), 0) == -1)
        {
#if DEBUG_FILE
          if (debug_file)
            {
              fprintf (debug_file, "mq_send failed: %d (%s)\n", errno,
                       strerror (errno));
              fflush (debug_file);
            }
#endif
...
signal_handler (int sig)
{
  ...
  end_program = 1;
  fprintf (stderr, "signal_handler: got signal %d, set end_program=1\n", sig);
  return;
}
...
int
mac_index (uint8_t *mac, struct UAV_RID *RID_data)
{
          ...
          if (uav->mac[0])
            {
              ...
              fprintf (
                  stderr,
                  " - reusing RID record %d (%02x:%02x:%02x:%02x:%02x:%02x)\n",
                  oldest, uav->mac[0], uav->mac[1], uav->mac[2], uav->mac[3],
                  uav->mac[4], uav->mac[5]);
            }
          else
            {
...
void
dump (char *name, uint8_t *data, int len)
{
...
char *
printable_text (uint8_t *data, int len)
{
  ...
  for (i = 0; (i < 31) && (i < len); ++i)
    {
      ...
      text[i] = (isprint (data[i]) && (data[i] != '"') && (data[i] != '\\'))
                    ? (char)data[i]
                    : '.';
    }
...
#define UDP_BUFFER_SIZE                                                       \
  800 // Should really be 500, but we can get close to 500 with packed
      // messages.

int
write_json (char *json)
{
      ...
      while (*json)
        {
          ...
          if ((c == 0x0a) || (c == 0x0d) || (index >= UDP_BUFFER_SIZE))
            {

              udp_buffer[index] = 0;

              if ((status
                   = sendto (json_socket, udp_buffer, index, 0,
